 ...
 * @return  array   优惠劵数组
 */
function user_bonus($user_id, $goods_amount = 0,$team_first=false)
{
    $day    = getdate();
...
function order_query_sql($type = 'finished', $alias = '')
{
    ...
    if ($type == 'finished')
    {
        return " AND {$alias}order_status " . db_create_in(array(OS_CONFIRMED, OS_SPLITED)) .
               ...
               " AND {$alias}pay_status " . db_create_in(array(PS_PAYED,PS_PAYING ,3)) . " "; //, SS_SHIPPED
    }
	  /* 拼团待发货订单 */
            ...
            if($payment['pay_code']=='wxpay'){
                if($r['discount_type']==1&&$r['refund_sign']==0){//团长免单
                    ...
                    if($f){
                        ...
                        $weixin->send_wxmsg($openid, $_team_msg['refund_team_first']['title'], 'share.php?team_sign='.$team_sign , $_team_msg['refund_team_first']['desc']);
                    ...
                    if($f){
                        ...
                        $weixin->send_wxmsg($openid, $_team_msg['refund_team_discount']['title'],  'share.php?team_sign='.$team_sign , $_team_msg['refund_team_discount']['desc'] );
                    }
