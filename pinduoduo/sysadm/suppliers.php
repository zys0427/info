...
elseif (in_array($_REQUEST['act'], array('insert', 'update')))
{
    ...
    if ($_REQUEST['act'] == 'update')
    {
        ...
        /* 清空供货商的管理员 */
        $sql = "UPDATE " . $hhs->table('admin_user') . " SET suppliers_id = 0, action_list = '" . SUPPLIERS_ACTION_LIST . "' WHERE suppliers_id = '" . $suppliers['id'] . "'";
        $db->query($sql);

        /* 添加供货商的管理员 */
        if (isset($_POST['admins']))
        {
            $sql = "UPDATE " . $hhs->table('admin_user') . " SET suppliers_id = '" . $suppliers['old']['suppliers_id'] . "' WHERE user_id " . db_create_in($_POST['admins']);
            $db->query($sql);
        }
...
function suppliers_list()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数 */
        $sql = "SELECT COUNT(*) FROM " . $GLOBALS['hhs']->table('suppliers') . $where;
